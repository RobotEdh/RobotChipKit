...
//	Cache.IPAddr.Val = 0x0;
	Cache.IPAddr.Val = 0x0FFFFFFFF; // prevent matching in ARPIsResolved
}
#endif
...
BOOL ARPIsResolved(IP_ADDR* IPAddr, MAC_ADDR* MACAddr)
{
    ...
    {
#if 0   // test fix, fixed by initialization
        unsigned char rgbBroadcast[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
        if(memcmp(&Cache.MACAddr, rgbBroadcast, 6) == 0)
        {
            return FALSE;
        }
#endif
